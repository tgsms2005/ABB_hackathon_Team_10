    ...
    <div class="container">
      <div class="range-cards-container">
        <mat-card class="range-card">
          <mat-card-title>Training Period</mat-card-title>
        ...
        <mat-card class="range-card">
          <mat-card-title>Testing Period</mat-card-title>
          <ng-container *ngIf="showFields">
            <mat-form-field appearance="fill">
              <mat-label>Start Date</mat-label>
        ...
        <mat-card class="range-card">
          <mat-card-title>Simulation Period</mat-card-title>
          <ng-container *ngIf="showFields">
            <mat-form-field appearance="fill">
              <mat-label>Start Date</mat-label>
      ...
      <div class="actions">
        <button mat-raised-button color="primary" (click)="validateRanges()">Validate Ranges</button>
      </div>
      
      <div *ngIf="validationStatus" class="validation-message">
        <mat-icon [color]="validationStatus === 'Valid' ? 'accent' : 'warn'">{{ validationStatus === 'Valid' ? 'check_circle' : 'warning' }}</mat-icon>
        <span [class.success-text]="validationStatus === 'Valid'" [class.error-text]="validationStatus !== 'Valid'">
          {{ validationMessage }}
        </span>
      </div>

      <div *ngIf="validationStatus === 'Valid'" class="summary-container">
        <p>Training Records: {{ recordCounts.trainingCount }}</p>
        <p>Testing Records: {{ recordCounts.testingCount }}</p>
        <p>Simulation Records: {{ recordCounts.simulationCount }}</p>
      </div>
    </div>
  `,
  styles: [`
